...
fn update_marker_system(
    ...
    query_new.iter_mut().for_each(|(entity, pos, vel)| {
        if is_in_range(pos, vel, &config) {
...
fn dont_write_to_written_once_atoms_system(
    ...
    if step.n % interval.0 != 0 {
        return;
    }
    query.iter_mut().for_each(|mut marker| {
...
impl Plugin for MarkerPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(
            PreUpdate,
            update_marker_system.after(CollisionsSet::WallCollisionSystems),
        );
